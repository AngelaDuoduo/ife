window.onload = function() {
	...
	*/
		...
		if (event.keyCode !== 13) {
			...
			ajax("http://localhost:3000", {
				...
				onsuccess: function(responseText) {
					suggestData = responseText.split(/[\[\],\s'"]+/).filter(function(elem) {
						return elem !== "";
					});
	...
	$.on(document, "keyup",function(event) {
		var activeTip = $(".active") || (suggestData && tips[suggestData.length - 1]) || null;
		for (var pos = 0; pos < tips.length; pos++) {
			if (activeTip === tips[pos]) {
				break;
			}
		}
		if (activeTip && event.keyCode == 38) {
		...
		} else if (activeTip && event.keyCode == 40) {
			removeClass(activeTip, "active");
